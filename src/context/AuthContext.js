...
export function AuthProvider({ children }) {
  ...
  const loginWithGoogle = useCallback(async () => {
    try {
      ...
      if (Platform.OS === 'web') {
        // Browser OAuth must navigate the current tab so Supabase can restore
        // the returned session from the URL. The auth state listener below
        // completes the existing login flow without changing native behavior.
